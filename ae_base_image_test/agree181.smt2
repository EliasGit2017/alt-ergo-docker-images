...
(assert (not (=> (and (forall ((n node$type)) (=> (= (select chosen n) Truth) (exists ((m node$type)) (= (select output n) (select input m)) )) ) (forall ((n node$type)) (=> (= (select init_done n) Truth) (not (= (select global_state n) Values$empty))) )) (=> (and (= phase send_phase) (forall ((n node$type) (m node$type)) (= (select (select send_done n) m) Truth) )) (=> (= phase$1 recv_phase) (forall ((n node$type)) (=> (= (select chosen n) Truth) (exists ((m node$type)) (= (select output n) (select input m)) )) ))))))
(check-sat)
(exit)
